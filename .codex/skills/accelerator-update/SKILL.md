...
Read `.codex/.accelerator-version` file:

```bash
cat .codex/.accelerator-version
```
...
Read `.codex/.accelerator-repo` file to get the repository name:

```bash
cat .codex/.accelerator-repo 2>/dev/null || echo ""
```
...
```bash
python3 .codex/skills/accelerator-update/scripts/compare_versions.py "<current>" "<latest>"
```
...
find /tmp/accelerator-update-preview/.codex -type f | while read file; do
...
This creates .codex.backup.<timestamp> directory.
...
```bash
ls -la /tmp/accelerator-update/.codex/
```
...
# Skills
rm -rf .codex/skills
cp -r /tmp/accelerator-update/.codex/skills .codex/

# Agents
rm -rf .codex/agents
cp -r /tmp/accelerator-update/.codex/agents .codex/

# Commands
rm -rf .codex/commands
cp -r /tmp/accelerator-update/.codex/commands .codex/
```

#### 9.2: Merge Settings

```bash
python3 .codex/skills/accelerator-update/scripts/merge_settings.py \
  .codex/settings.json \
  /tmp/accelerator-update/.codex/settings.json \
  .codex/settings.json.merged

mv .codex/settings.json.merged .codex/settings.json
```
...
```bash
# Check for new hooks
find /tmp/accelerator-update/.codex/hooks -type f 2>/dev/null | while read hook; do
...
```bash
cp /tmp/accelerator-update/.codex/.accelerator-version .codex/
echo "<REPO>" > .codex/.accelerator-repo
```
...
ls -d .codex/skills .codex/agents .codex/commands

# Verify settings is valid JSON
python3 -m json.tool .codex/settings.json > /dev/null
```
...
- .codex/agents/existing-agent.md
- ...

### Preserved

- .codex/settings.local.json (user file)
- .codex/skills/my-custom-skill/ (user skill)
- ...
...
Created backup at: .codex.backup.<timestamp>
```
